import pyodbc
import platform
from fhcrc_clinical.SocialHistories.DataLoading.DataLoadingGlobals import TEST_JOB_IDS, TRAIN_JOB_IDS, IAA_JOB_IDS
...
def get_labkey_data(data_div):
    ''' Returns test data in labkey derived from the set of job_ids defined in the DataLoadingGlobals.py file'''
    cur = __get_server_connection()
    results = __execute_data_pull(cur, data_div)
    patient_doc_annotations = __organize_data(results)
    return patient_doc_annotations


def get_IAA_data():
    '''Returns the data used for IAA calculations'''
    cur = __get_server_connection()
    results = __execute_data_pull(cur, "IAA")
    patient_doc_annotations = __organize_data(results)
    return patient_doc_annotations
